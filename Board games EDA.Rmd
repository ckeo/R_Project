...
#84 different categories of boardgames. Which are the top 20?
...
df %>%
  ...
  arrange(desc(perc)) %>%
  filter(perc > .015) %>%
  ggplot(aes(x = fct_reorder(category, perc), y = perc)) + geom_col() + labs(title = "Top Game Categories (Make up over 1.5% of all categories)") + scale_x_discrete("Category") + scale_y_continuous("Percent of Total Categories") +coord_flip()
...
ggplot(cat_stats, aes(x = fct_reorder(Category, mean_rating), y = mean_rating)) + geom_bar(position = position_dodge(), stat = 'identity', fill = wes_palette(n=5, name="FantasticFox1")[3]) + geom_errorbar(aes(ymin = mean_rating - sd_rating, ymax = mean_rating + sd_rating), width = .2) + coord_flip() + labs(title = "Average User Scores by Game Category") + scale_x_discrete(name="Board Game Category")  + scale_y_continuous(name = "Average User Rating")
...
#neither of them are really great but average seems to predict rating better than it predicts owned.
